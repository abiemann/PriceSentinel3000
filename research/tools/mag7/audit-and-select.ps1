...
while ($null -ne $repoDirectory -and -not (Test-Path -LiteralPath (Join-Path $repoDirectory.FullName 'research/strategies/Netflix (NFLX) Confirmation - experimental.thinkscript'))) {
...
$logic = Canonical-Logic (Get-Content -Raw -LiteralPath (Join-Path $repo 'research/strategies/Netflix (NFLX) Confirmation - experimental.thinkscript'))
