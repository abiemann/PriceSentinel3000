...
```powershell
dotnet build research/tools/low-price/ReplayExport.csproj -c Release
```

Run from the repository root, substituting the actual manifest and output paths:

```powershell
& './research/tools/low-price/bin/Release/net10.0-windows/ReplayExport.exe' `
  --manifest './archive/research/low-price-2026-09-06/development-baseline-jobs.json' `
...
```powershell
python research/tools/low-price/audit-indicators.py `
  --directory artifacts/low-price-research/development `
  --output artifacts/low-price-research/audit-current/development-indicator-audit.json
python research/tools/low-price/compare-replays.py `
  --reference artifacts/low-price-research/development `
  --verification artifacts/low-price-research/verification `
  --output artifacts/low-price-research/audit-current/verification-comparison.json
```
