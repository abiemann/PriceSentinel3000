...
$assembly = [Reflection.Assembly]::LoadFrom((Join-Path $repository 'research/tools/low-price/bin/Release/net10.0-windows/ReplayExport.dll'))
...
$files = @(Get-ChildItem (Join-Path $repository 'artifacts/low-price-research/development') -Filter '*-C1.json' |
    Where-Object Name -Match '^(SOFI|F|RIVN|SNAP|NU|CCL|AAL|PINS)-2026-09-0[23]-C1\.json$')
...
$output = Join-Path $repository 'artifacts/low-price-research/export-validation'
New-Item -ItemType Directory -Path $output -Force | Out-Null
[pscustomobject]@{generatedAtUtc=[datetimeoffset]::UtcNow.ToString('O');checks=$checks;appCalls=0} | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $output 'results.json')
