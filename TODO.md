...
At the September 10 source review, refreshed Windows CI, release packaging, checksums, and source-provenance verification were pending. The [rebuilt release](https://github.com/abiemann/PriceSentinel3000/releases/tag/1.3) and its attached provenance record provide the final publication results; local validation and the historical runs below do not complete those checks.
...
- [x] Initial publication validation: local Release build with zero warnings/errors and all 1,404 tests passing; successful [Windows CI](https://github.com/abiemann/PriceSentinel3000/actions/runs/34422818384) and [Release workflow](https://github.com/abiemann/PriceSentinel3000/actions/runs/34422844466). These historical runs do not validate the September 10 rebuild.
- [x] September 10 rebuild source review: document daily gap attempts and valid candle revisions, persistent older-empty-day boundaries and saved-queue recovery, discovery progress and pause handling, and Eastern-date timelines displayed in local time. The current source passed all 1,495 tests and a local Release build with zero warnings/errors.
...
- [x] After the published 1.3 rebuild: keep at most eight unfinished stock/date download items active, persist their selection across retry waits, pauses, and restarts, and admit another item when a slot opens. Preserve serial broker requests, calendar checks, and the shared date frontier.
