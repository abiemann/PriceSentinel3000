...
Reviewed September 10, 2026 for rebuilding the published [release 1.3](https://github.com/abiemann/PriceSentinel3000/releases/tag/1.3) from the current source. PriceSentinel provides broker-isolated Paper Trader and Replay modes, real Robinhood market data, documented bid/ask and replay-close fills, risk controls, SQLite WAL journaling, optional immediate or weekday T+1 settlement, external strategies, and MCP control with candle/indicator/event telemetry. Historical Replay preserves the actual source duration when falling back from 15-second to 30-second or one-minute data. The rebuilt 1.3 includes the implemented controls and retention changes below. The roadmap separates completed source validation from pending publication checks, supervised runtime checks, and future work.

## Release 1.3 validation and remaining runtime checks

At this September 10 source review, refreshed Windows CI, release packaging, checksums, and source-provenance verification were pending for the newly selected source. The [rebuilt release](https://github.com/abiemann/PriceSentinel3000/releases/tag/1.3) and its attached provenance record provide the final publication results; local validation and the historical runs below do not complete those checks.
...
- [x] Initial publication validation: local Release build with zero warnings/errors and all 1,404 tests passing; successful [Windows CI](https://github.com/abiemann/PriceSentinel3000/actions/runs/34422818384) and [Release workflow](https://github.com/abiemann/PriceSentinel3000/actions/runs/34422844466). These historical runs do not validate the newly selected rebuild source.
- [x] Earlier September 10 rebuild source at `dff0b41`: document daily gap attempts and valid candle revisions, persistent older-empty-day boundaries and saved-queue recovery, discovery progress and pause handling, and Eastern-date timelines displayed in local time. That source passed all 1,495 tests and a local Release build with zero warnings/errors; this is historical validation.
- [x] Latest September 10 rebuild source review: include 1x-500x Replay with MAX and speed clamping, CHECK placement, existing Replay autocomplete connections, idle symbol display resets, eight active retained-download items, completed-time coverage refresh, and the shared coverage popup with pinned-folder downloads. A clean local Release build passed with zero warnings/errors, all 1,545 tests passed, and the retention UI was checked at normal and minimum window sizes.
...
- [x] Included in rebuilt 1.3: clear the previous symbol's captured chart, prices, and paper-account display when selecting a different symbol while idle, preserving completed journal and structured session results.

- [x] Included in rebuilt 1.3: keep an existing Robinhood connection available for autocomplete after local Replay, while preserving disconnected offline playback and shared background downloads.

- [x] Included in rebuilt 1.3: increase paced Replay to 500x, add **MAX** beside the speed input to select 500x, and place **CHECK** to the left of the availability guidance.
...
- [x] Keep unavailable 15-second ranges and attempt counts in the daily JSON, including unresolved portions of partial responses. Always queue today; precheck older dates before queueing, allow two normal attempts per gap, and stop each ticker after a complete older regular-day gap pass returns no broker candles, even with saved local data. Remember the boundary in daily JSON and recover it from completed checks in an older saved queue before issuing more requests. Force bypasses the cap; saved portions clear their attempt records.
- [x] Scheduled runs and the date-free **Download gaps now** action collect all available regular, premarket, after-hours and overnight 15-second history, reuse saved coverage, collect today's completed candles, check older gaps in hourly windows before queuing remaining work, skip complete days, and stop each equity at the first older regular trading date whose entire gap pass returns no broker candles.
...
- [x] Included in rebuilt 1.3: keep at most eight unfinished stock/date download items active, persist their selection across retry waits, pauses, and restarts, and admit another item when a slot opens. Preserve serial broker requests, calendar checks, and the shared date frontier.
...
- [x] Continue ready download batches without a fixed pause; keep collection running after closing its window, with live progress inside the header button and current status on reopening. Show the actual ticker/date and busy indicators during older-history prechecks, even after all retained rows finish; honor pause between local checks while preserving the queue.
...
- [x] Included in rebuilt 1.3: align daily download-state coverage with Local library's completed-candle cutoff, excluding future time and market closures. Refresh from validated metadata during downloads, rescans, restarts, and clock changes; retain selected-range scope, fixed request cutoffs, and failed/unavailable indicators.
...
- [x] Anchor saved-coverage timelines to the selected Eastern date and display their boundaries as local dates and times, retaining elapsed overnight candles before local midnight. Distinguish saved, partial, missing, closed, and future intervals, and calculate today's coverage only through completed candles at the current time.
- [x] Included in rebuilt 1.3: open the shared saved-coverage popup from Schedule & Downloads rows by click or Enter, with the same green selection as Local Library. Keep coverage and selected-block downloads pinned to the queued job's original library folder.
