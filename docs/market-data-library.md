...
A small SQLite index, `.collection-gaps.sqlite3`, lives in the library folder and remembers successful broker requests that returned no candles. **Download gaps now** checks the saved candle files first, then subtracts these known empty ranges before making requests. Requests do not bridge saved candles or indexed empty blocks. This lets an unfillable hole produce its own empty response instead of repeatedly returning neighboring saved candles. Entries are scoped to the broker, instrument, date, session and adjustment identity; overlapping or adjacent ranges are consolidated. The index also retains positive availability evidence so a previously productive day whose remaining gaps are remembered does not prevent checking older dates.
